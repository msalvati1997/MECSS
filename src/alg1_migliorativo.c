...
void find_batch_b() {
    ...
    for (b; b <= BATCH_B; b = b + 1) {
        ...
        sprintf(filename,"results/alg1_migliorativo/infinite/rt_batch_inf_%d_.csv", b);
        FILE *csv;
...
void write_rt_csv_infinite() {
    char * filename= "results/alg1_migliorativo/infinite/rt_infinite.csv";
    char * filename_ploss ="results/alg1_migliorativo/infinite/ploss_infinite_slot.csv";
    ...
    for (int i = 0; i < NUM_BLOCKS ; i++) {
        ...
        sprintf(filename_delays,"results/alg1_migliorativo/infinite/infinite_dl_%s.csv", stringFromEnum(i));
        FILE *csv_delays;
     ...
     for (int i = 0; i < NUM_BLOCKS ; i++) {
        ...
        sprintf(filename_wait,"results/alg1_migliorativo/infinite/infinite_wait_%s.csv", stringFromEnum(i));
        FILE *csv_wait;
...
void initialize() {
   ...
   for(int i=0;i<blocks[2].num_servers;i++) {
    ...
    for(int i=0; i<blocks[3].num_servers; i++){
        (*enode_unit+i)->status=IDLE;
        (*enode_unit+i)->online=ONLINE;
        (*enode_unit+i)->loss=NOT_LOSS_SYSTEM;
        (*enode_unit+i)->stream=streamID++;
        (*enode_unit+i)->block=&blocks[3];
        streamID+=streamID;
   ...
   insertSorted(&global_sorted_completions, (compl) {(*enode_unit+1), INFINITY});
...
void allocate_memory() {
   ...
   (*wlan_unit)=calloc(2,sizeof(server));
   enode_unit=calloc(2,sizeof(server*));
   (*enode_unit)=calloc(2,sizeof(server));
   ...
   (*enode_unit+1)->block=malloc(sizeof(block));
   ...
   blocks[3].serv = malloc(sizeof(server*)*2);
...
void write_rt_csv_finite() {
    FILE *csv;
    FILE *csv2;
    char* filename = "results/alg1_migliorativo/finite/finite_results.csv";
    csv = open_csv(filename);
    if(csv != NULL){
    ...
    for(int i=0;i<NUM_BLOCKS;i++) {
      ...
      sprintf(filename2,"results/alg1_migliorativo/finite/finite_results_for_%s.csv",stringFromEnum(i));
